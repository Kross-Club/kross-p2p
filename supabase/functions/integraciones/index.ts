...
import { chequearTwilio, enviarSms } from '../_shared/sms.ts'
...
async function pingDe(id: Proveedor, llaves: Record<string, string | null>): Promise<boolean | null> {
  ...
  // Twilio trae su propio chequeo (`chequearTwilio`): lista un mensaje, que es
  // una lectura sobre lo mismo que escribimos, y anota el motivo si rebota.
  if (id === 'TWILIO') return chequearTwilio()
  if (id === 'OLVA_LAT') {
