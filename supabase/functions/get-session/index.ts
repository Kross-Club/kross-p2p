...
Deno.serve(async (req) => {
  ...
  const { data: session, error } = await supabase
    ...
    .select(`
      ...
      dispatch_type, origin_store_id, reparto_lima, agency_name, agency_branch_id, delivery_reference,
  ...
  const { data: cobros } = viewerIsSeller
    ? await supabase.from('cobros')
        .select('id, tipo, monto, estado, matched_at, pay360_coupon_id, pay360_consumer_code, flow_token, coupon_expires_at, concepto, created_by, created_at, comision_pen, costo_pasarela_pen')
